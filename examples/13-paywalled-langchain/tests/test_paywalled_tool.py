"""
Tests for ErgoPaidTool — exercise the wire-level contract without
spinning up the bridge daemon, the seller's API, or LangChain.
...
# Make the example importable without having LangChain installed.
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from ergo_agent_pay import BridgeClient, ErgoAgentPayError

from buyer_tool import ErgoPaidTool, ToolConfig, build_expected_output  # noqa: E402
...
def make_config(
    ...
    *,
    ...
    def fake_http(method: str, url: str, body: bytes, headers: dict[str, str]) -> dict:
        ...
        # Return responses in order, repeating the last one if more calls happen.
        idx = len(captured) - 1
...
class PolicyGateTest(unittest.TestCase):
    def test_blocked_recipient_short_circuits_before_bridge(self) -> None:
        ...
        tool = ErgoPaidTool().bind_config(config)
