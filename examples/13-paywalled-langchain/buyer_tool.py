"""
ErgoPaidTool — a LangChain tool whose `_run` is paywalled by an Ergo Note.
...
Each invocation:
  ...
  4. returns the response body to the agent loop

The entire SDK / bridge / API surface is configurable so the tool is
...
try:  # pragma: no cover — only required at runtime, not at test time
...
class ToolConfig:
    ...
    # Test seam — defaults to urllib.request.urlopen.
    http_call: Callable[[str, dict, bytes, dict[str, str]], dict] | None = None


class ErgoPaidToolInput(BaseModel):
    text: str = Field(default="", description="Text to send to the paid endpoint.")


class ErgoPaidTool(BaseTool):
    """LangChain tool whose execution is paywalled by an Ergo Note."""

    name: str = "ergo_paid_summarise"
    description: str = (
        "Pay for a summarisation of the supplied text. Each call costs a Note "
        "issued from the agent's reserve. The seller's API verifies the Note "
        "on-chain before serving the request."
    )
    args_schema: type[BaseModel] = ErgoPaidToolInput

    # Lazily injected — set via `bind_config`.
    _config: Optional[ToolConfig] = None

    def bind_config(self, config: ToolConfig) -> "ErgoPaidTool":
        self._config = config
        return self
    ...
    def _run(self, text: str) -> str:  # type: ignore[override]
        cfg = self._config
        if cfg is None:
            raise RuntimeError("ErgoPaidTool not bound — call bind_config(...) first")
        ...
        # Pre-commit to the expected output BEFORE issuing the Note.
        # The seller must produce a response whose serialised form
        # matches `expected` for the Note to be redeemable.
        ...
        # The unsigned-tx flow doesn't tell us the boxId. In a real
        # deployment, watch the node for the confirming TX and read
        # the output's boxId from there. For demos we accept an
        # override via env.
...
def build_expected_output(text: str) -> str:
    ...
    Note to be redeemable. Keeping this deterministic is the agent's
