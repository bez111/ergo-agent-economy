"""
Budget gate that lifts the SDK's PolicyEngine model into the LangChain
boundary. Used by ErgoPaidTool to refuse a Note issuance before it
reaches the bridge daemon.

Mirrors the contract / hook surface of `ergo-agent-pay/src/policy.ts` so
behaviour stays consistent across language SDKs.
"""
...
class PaymentPolicy:
    """
    Mirror of the v2 PolicyEngine. nano units (e.g. nanoERG / 1e-6 USDT).
    ...
    def record_after(self, recipient: str, value: int) -> None:
        del recipient  # unused, but kept for symmetry with the TS engine
