...
| P4 Testnet pilots | Started | pilot matrix, result template, testnet wallet setup, per-rail rollback plans, first local mock pilot result, machine-checked completed/pending pilot status, and Ergo testnet env preflight committed; external testnet pilot evidence still pending |
