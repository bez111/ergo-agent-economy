...
## The mainnet guardrail

The SDK enforces a simple rule before broadcasting:

| Network | `scriptErgoTree` | `allowInsecureDevMode` | Behaviour |
|---|---|---|---|
| `testnet` | any | any | always allowed |
| `mainnet` | set | any | allowed |
| `mainnet` | missing | `true` | allowed (explicit opt-in) |
| `mainnet` | missing | not set / `false` | **rejected** with `INSECURE_MAINNET_MODE` |

The opt-in flag is on the agent config:

```ts
new ErgoAgentPay({
  address,
  network: "mainnet",
  allowInsecureDevMode: true, // I have read SECURITY.md and accept P2PK semantics
})
```
...
The error code is `INSECURE_MAINNET_MODE`. The CLI exits 3 in that case so
shell pipelines can tell it apart from a network failure (1) or an argument
error (2).
...
3. Add a small wrapper around the SDK that loads your pinned ergoTrees and
   passes them to `createReserve` / `issueNote` / `deployTracker`. Never
   construct a mainnet `ErgoAgentPay` instance with `allowInsecureDevMode:
   true` in normal code.
4. Run the same end-to-end flow against mainnet with a tiny Reserve and a
   tiny Note before you scale up. The guardrail will refuse to broadcast if
   you forgot to wire the ergoTree.
...
If you find yourself reaching for `allowInsecureDevMode: true` because
something else is in the way, treat that as a signal to stop and fix the
upstream issue rather than disabling the guardrail.
