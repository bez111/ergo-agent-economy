...
| M-003 | Medium | ⛔ residual | Tracker DoS via `creationInfo._1` requires a different tracker box format (explicit `lastSignedAt` register). v1 design item; not changed in this PR because it changes the tracker contract surface, not just the reserve. |
...
| M-007 | Medium | ⛔ residual | Empty `trackerSigBytes` triggering emergency mode is a contract-level encoding choice. SDK side will refuse to emit empty bytes when a basis-redemption builder lands; out of scope for this PR. |
